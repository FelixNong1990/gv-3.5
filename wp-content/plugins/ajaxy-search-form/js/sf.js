(function($) {
    ...
    $.ajaxyLiveSearch = {
		...
		load: function( elem, options, arg ) {
				...
				if($('#sf_sb').length == 0){
					$('body').append('<div id="sf_sb" class="sf_sb">'+
		...
		loadResults: function(object) {
			...
			if(jQuery(elem).val() != "" && jQuery.trim(jQuery(elem).val()).length > 1)
			{
				...
				if(options.search) {
					...
					if (cache[form_data]) {
						jQuery("#sf_val").html('<ul class="sf_main">'+cache[form_data]+'</ul>');
						$.ajaxyLiveSearch.loadLiveEvents(object);
						jQuery("#sf_sb").show();
						jQuery("#sf_val li.sf_lnk").eq(0).addClass('sf_selected').focus();
					} else {
		...
		bounds: function (elem, options) {
			...
			if(offset) {
				return {top: offset.top, left: offset.left + options.leftOffset, bottom: offset.top +  jQuery(elem).innerHeight() + options.topOffset - 5, right: offset.left - jQuery('#sf_sb').innerWidth() + jQuery(elem).innerWidth()};			
			}
		},
		...
		loadLiveEvents: function(object){
			...
			jQuery("#sf_val li.sf_lnk").mouseover(function(){
				start = jQuery(this).index('.sf_lnk');
