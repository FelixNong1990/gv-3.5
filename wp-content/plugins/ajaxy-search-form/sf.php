...
class AjaxyLiveSearch {
	...
	function get_search_objects($all = false, $objects = false, $specific_post_types = array(), $specific_taxonomies = array(), $specific_roles = array())
	{
		...
		$arg_category_show = isset($_POST['show_category']) ? $_POST['show_category'] : 0;
		...
		$arg_post_category_show = isset($_POST['show_post_category']) ? $_POST['show_post_category'] : 0;
		...
		switch($type) {
			...
			case 'post_type':
				...
				if(!$template_post && in_array($template, self::$woocommerce_post_types)) {
					$template_post = '<a href="{post_link}">{post_image_html}<span class="sf_text">{post_title} - {price}</span><span class="sf_small">by {post_author} on {post_date_formatted}</span></a>';
				}elseif(!$template_post){
					$template_post = '<a href="{post_link}">{post_image_html}<span class="sf_text">{post_title} </span><span class="sf_small">by {post_author} on {post_date_formatted}</span></a>';
				}
				break;
			default:
				$template_post = get_option('sf_template_'.$template);
	...
	function post_object($id, $term_id = false, $matches = false) {
		...
		if($post != null)
		{
			...
			if(in_array('{post_image}', $matches) || in_array('{post_image_html}', $matches)) {
				$src = getImgSrc($post->ID);
						
				$post_object->post_image_html = '<img src="'. $src .'" width="'.$size['width'].'" height="'.$size['height'].'"/>';
			...
			if(in_array('{post_title}', $matches)) {
