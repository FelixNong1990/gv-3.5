		...
		*/
		...
		public function kksr_ajax()
		{
			...
			else
			{
				...
				$legend = str_replace('[avg]', number_format((float)($avg*($total_stars/5)), 2, '.', ''), $legend);
		...
		public function grs_legend($legend, $id)
		{
			if(parent::get_options('kksr_grs'))
			{
				...
				if($score)
				{
					...
					$leg = str_replace('[avg]', '<span property="v:average">'.$avg.'</span>', $leg);
					...
					$snippet .= '<span class="tooltip-rating" rel="v:rating">';
