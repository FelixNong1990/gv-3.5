(function($) {
	...
	$(document).ready(function() {
		...
		$.each(jqueryUIVersion, function(index, value) {
			jqueryUIVersion[index] = parseInt(value, 10);
		});
		...
		$('.slider-pro #add-new-slides').click(function(event) {
			...
			var url = $.url($(this).prop('href')),
				...
				position = parseInt($('.slider-pro .slidebox').length, 10) + 1,
				counter = 0,
				quantity = parseInt($('.slider-pro #add-slides-quantity').val(), 10);
			
			// find the index of the new panel
			$('.slider-pro .slidebox').each(function(index) {
				counter = Math.max(counter, parseInt($(this).find('.counter').val(), 10));
			});
				...
				if (typeof wp != 'undefined' && wp.media && wp.media.editor) {
					wp.media.editor.send.attachment = function(props, attachment) {
						...
						if (sp_js_vars.rewrite_multisite_paths) {
							var imagePath = url.split('/files/');
		
							var domain = url.substring(7).split('/')[0];
							
							if (imagePath[1])
								url = 'http://' + domain + sp_js_vars.current_site_path + 'wp-content/blogs.dir/' + sp_js_vars.blog_id + '/files/' + imagePath[1];
						}

						selectedImages.push( { url:url, alt: alt, title: title } );
					};
						...
						$.ajax({
							...
							complete: function(data) {
								...
								slides.each(function(index) {
									...
									slide.find('.path').first().val(image.url);
									slide.find('.alt').first().val(image.alt);
									slide.find('.title').first().val(image.title);
		...
		$('.slider-pro .preview-slider').click(function(event) {
			...
			var url = $.url($(this).prop('href')),
				...
				width = fluidWidth ? 700 : parseInt(paramWidth, 10),
				height = fluidHeight ? 500 : parseInt(paramHeight, 10),
				modalWindow,
			...
			function sizeLiveUpdate() {
				var widthDiff = parseInt(sliderWidthInput.val(), 10) - parseInt(slideWidthInput.val(), 10),
					heightDiff = parseInt(sliderHeightInput.val(), 10) - parseInt(slideHeightInput.val(), 10);
					
				
				sliderWidthInput.bind('propertychange input keyup paste', function(event) {
					slideWidthInput.val(parseInt($(this).val(), 10) - widthDiff);
				});
				
					
				slideWidthInput.bind('propertychange input keyup paste', function(event) {
					sliderWidthInput.val(parseInt($(this).val(), 10) + widthDiff);
				});
				
				
				sliderHeightInput.bind('propertychange input keyup paste', function(event) {
					slideHeightInput.val(parseInt($(this).val(), 10) - heightDiff);
				});
				
				
				slideHeightInput.bind('propertychange input keyup paste', function(event) {
					sliderHeightInput.val(parseInt($(this).val(), 10) + heightDiff);
				});
			}
	...
	function prepareSlide(slide) {
		...
		if (jqueryUIVersion[0] == 1 && jqueryUIVersion[1] < 9) {
			slide.find('.slide-tabs').tabs({
				...
				create: function() {
					$(this).tabs('select', parseInt(slide.find('.selected-tab').val(), 10));
				}
			});
		} else {
			slide.find('.slide-tabs').tabs({
				...
				create: function() {
					$(this).tabs('option', 'active', parseInt(slide.find('.selected-tab').val(), 10));
				}
			});
		}
		...
		slide.find('.duplicatediv').click(function() {
			...
			var action = 'sliderpro_duplicate_slide',
				id = parseInt(slide.find('.id').val(), 10),
				position = parseInt($('.slider-pro .slidebox').length, 10) + 1,
				counter = 0;
		...
		slide.find('a.preview-button').click(function(event) {
			...
			var imagePath = $(this).parents('.panel-tab-content').find('.path').val();
			
			if (imagePath.indexOf('[') != -1)
				return;
			
			var	box = $(this).parents('.panel-tab-content').find('.preview-box'),
				w = parseInt(box.css('width'), 10),
				h = parseInt(box.css('height'), 10);
		...
		slide.find('a.open-media-loader').click(function(event) {
			...
			if (sp_js_vars.custom_media_loader || !sp_js_vars.is_new_media_loader) {
				...
				window.send_to_editor = function(html) {
					if (html !== '') {
						var imageUrl = $(html).attr('src') || $('img', html).attr('src');

						// add the image url into the 'Path' field
						button.parents('.panel-tab-content').find('.path').val(imageUrl);
				
						// trigger the click event in order to display the image
						button.parents('.panel-tab-content').find('.preview-button').trigger('click');
					}
				};
				...
				if (typeof wp != 'undefined' && wp.media && wp.media.editor) {
					wp.media.editor.send.attachment = function(props, attachment) {
						...
						// add the image url into the 'Path' field
						button.parents('.panel-tab-content').find('.path').val(url);
						button.parents('.panel-tab-content').find('.alt').val(alt);
						button.parents('.panel-tab-content').find('.title').val(title);
		...
		slide.find('.layer-editor').each(function(event) {
			...
			editor.find('.layer-item').each(function() {
				createLayerItem(viewport, slideCounter, parseInt($(this).data('id'), 10), $(this));
			});
			
			
			var layerCounter = parseInt(layersNum.val(), 10);
		...
		slide.find('.slide-tabs-settings label').each(function() {
			var label = $(this),
				...
				currentValue = parseInt(override.val(), 10);
		...
		layerItem.draggable({
					stop: function() {
						...
						var horizontalPosition = position.indexOf('right') != -1 ? 'right' : 'left',
							...
							horizontalPositionValue = parseInt(layerItem.css(horizontalPosition), 10) + parseInt(layerItem.css('margin-' + horizontalPosition), 10),
							verticalPositionValue = parseInt(layerItem.css(verticalPosition), 10) + parseInt(layerItem.css('margin-' + verticalPosition), 10);
				...
				.mousedown(function() {
					layerItem.css('z-index', parseInt(layerItem.css('z-index'), 10) + 1000);
				})
				.dblclick(function() {
					viewport.find('.textarea-focus').each(function() {
						var textarea = $(this),
	...
	function mediaLoaderProcessContent(allow) {
		...
		mediaContainer.find('a.insert-image').click(function(event) {
			...
			// add the image url into the 'Path' field
			mediaLoaderClickedButton.parents('.panel-tab-content').find('.path').val($(this).prop('href'));
