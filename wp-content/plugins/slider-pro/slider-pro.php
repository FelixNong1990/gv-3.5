...
define('SLIDER_PRO_VERSION', '3.9.2');
...
function sliderpro_create_menu() {
	...
	$restricted_pages = apply_filters('slider_pro_restricted_pages' , $restricted_pages);
	...
	if (!in_array('slider_pro', $restricted_pages)) {
	...
	if (!in_array('slider_pro_new', $restricted_pages)) {
	...
	if (!in_array('slider_pro_skin_editor', $restricted_pages)) {
	...
	if (!in_array('slider_pro_plugin_options', $restricted_pages)) {
	...
	if (!in_array('slider_pro_help', $restricted_pages)) {
...
function sliderpro_load_admin_scripts() {
	...
	if (get_option('slider_pro_use_compressed_scripts')) {
		wp_register_script('slider-pro-admin-js', plugins_url('/slider-pro/js/slider-pro-admin.js'), array('jquery'), SLIDER_PRO_VERSION);
		wp_register_script('slider-pro-colorpicker', plugins_url('/slider-pro/js/colorpicker.min.js'), array('jquery'), SLIDER_PRO_VERSION, true);
		wp_register_script('slider-pro-url-parser', plugins_url('/slider-pro/js/jquery.url.min.js'), array('jquery'), SLIDER_PRO_VERSION, true);
		wp_register_script('slider-pro-multiselect', plugins_url('/slider-pro/js/jquery.multiselect.min.js'), array('jquery'), SLIDER_PRO_VERSION, true);
		wp_register_script('slider-pro-multiselect-filter', plugins_url('/slider-pro/js/jquery.multiselect.filter.min.js'), array('jquery'), SLIDER_PRO_VERSION, true);
	} else {	
		wp_register_script('slider-pro-admin-js', plugins_url('/slider-pro/js/slider-pro-admin.js'), array('jquery'), SLIDER_PRO_VERSION);
		wp_register_script('slider-pro-colorpicker', plugins_url('/slider-pro/js/dev/colorpicker.js'), array('jquery'), SLIDER_PRO_VERSION, true);
		wp_register_script('slider-pro-url-parser', plugins_url('/slider-pro/js/dev/jquery.url.js'), array('jquery'), SLIDER_PRO_VERSION, true);
		wp_register_script('slider-pro-multiselect', plugins_url('/slider-pro/js/dev/jquery.multiselect.js'), array('jquery'), SLIDER_PRO_VERSION, true);
		wp_register_script('slider-pro-multiselect-filter', plugins_url('/slider-pro/js/dev/jquery.multiselect.filter.js'), array('jquery'), SLIDER_PRO_VERSION, true);
	}
...
function sliderpro_load_admin_styles() {
	wp_enqueue_style('slider-pro-jquery-ui', plugins_url('/slider-pro/css/jquery-ui.css'), false, SLIDER_PRO_VERSION);
	wp_enqueue_style('slider-pro-colorpicker', plugins_url('/slider-pro/css/colorpicker.css'), false, SLIDER_PRO_VERSION);
	wp_enqueue_style('jquery-ui-multiselect', plugins_url('/slider-pro/css/jquery.multiselect.css'), false, SLIDER_PRO_VERSION);
	wp_enqueue_style('jquery-ui-multiselect-filter', plugins_url('/slider-pro/css/jquery.multiselect.filter.css'), false, SLIDER_PRO_VERSION);
...
function sliderpro_load_slider_styles() {
	...
	if (!empty($skins)) { 
		...
		$base_css = plugins_url('/slider-pro/css/slider/advanced-slider-base.css');

		if ($protocol == "https://" && strpos($base_css, 'https://') === false) 
			$base_css = str_replace('http://', 'https://', $base_css);

		wp_enqueue_style('slider-pro-slider-base', $base_css, false, SLIDER_PRO_VERSION);
		...
		foreach ($skins as $skin) { 
			if (!is_numeric($skin)) { 
				...
				$skin_css = $skin_obj['url'];

				if ($protocol == "https://" && strpos($skin_css, 'https://') === false) 
					$skin_css = str_replace('http://', 'https://', $skin_css);

				wp_enqueue_style($id, $skin_css, false, SLIDER_PRO_VERSION); 
			} else {
				$css_file = plugins_url('/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $skin . '/slider-pro-' . $skin . '-custom-css.css'); 
				$css_file_dir_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $skin . '/slider-pro-' . $skin . '-custom-css.css';

				if ($protocol == "https://" && strpos($css_file, 'https://') === false) 
					$css_file = str_replace('http://', 'https://', $css_file);
...
function sliderpro_get_page_styles() {
	...
	foreach ($ids_to_load as $id) {
		$slider_configs = $wpdb->get_results($wpdb->prepare("SELECT id, settings FROM " . $prefix . "sliderpro_sliders WHERE id = %d", $id), ARRAY_A);
...
*/
function sliderpro_load_slider_scripts( $load_jquery = false) {
	global $sliderpro_sliders_js, $sliderpro_scripts_to_load, $sliderpro_custom_scripts_to_load, $is_IE, $sliderpro_assets_dir;	
	...
	$deps = $load_jquery ? false : array('jquery');
	
	// load the excanvas script
	if (in_array('excanvas', $sliderpro_scripts_to_load) && $is_IE) {
		if (get_option('slider_pro_use_compressed_scripts'))
			wp_register_script('slider-pro-slider-excanvas-js', plugins_url('/slider-pro/js/slider/excanvas.compiled.js'), $deps, SLIDER_PRO_VERSION);
		else
			wp_register_script('slider-pro-slider-excanvas-js', plugins_url('/slider-pro/js/slider/dev/excanvas.js'), $deps, SLIDER_PRO_VERSION);
	...
	if (in_array('mousewheel', $sliderpro_scripts_to_load) && get_option('slider_pro_enqueue_jquery_mousewheel')) {
		if (get_option('slider_pro_use_compressed_scripts'))
			wp_register_script('slider-pro-slider-mousewheel-js', plugins_url('/slider-pro/js/slider/jquery.mousewheel.min.js'), $deps, SLIDER_PRO_VERSION);
		else
			wp_register_script('slider-pro-slider-mousewheel-js', plugins_url('/slider-pro/js/slider/dev/jquery.mousewheel.js'), $deps, SLIDER_PRO_VERSION);
	...
	if (in_array('lightbox', $sliderpro_scripts_to_load)) {
		if (get_option('slider_pro_use_compressed_scripts'))
			wp_register_script('slider-pro-lightbox-js', plugins_url('/slider-pro/js/slider/jquery.prettyPhoto.custom.min.js'), $deps, SLIDER_PRO_VERSION);
		else
			wp_register_script('slider-pro-lightbox-js', plugins_url('/slider-pro/js/slider/dev/jquery.prettyPhoto.custom.js'), $deps, SLIDER_PRO_VERSION);
	...
	if (in_array('css3_transitions', $sliderpro_scripts_to_load)) {
		if (get_option('slider_pro_use_compressed_scripts'))
			wp_register_script('slider-pro-css3-transitions-js', plugins_url('/slider-pro/js/slider/jquery.transition.min.js'), $deps, SLIDER_PRO_VERSION);
		else
			wp_register_script('slider-pro-css3-transitions-js', plugins_url('/slider-pro/js/slider/dev/jquery.transition.js'), $deps, SLIDER_PRO_VERSION);
	...
	if (in_array('vimeo_controller', $sliderpro_scripts_to_load)) {
		if (get_option('slider_pro_use_compressed_scripts'))
			wp_register_script('slider-pro-vimeo-controller-js', plugins_url('/slider-pro/js/slider/froogaloop.min.js'), $deps, SLIDER_PRO_VERSION);
		else
			wp_register_script('slider-pro-vimeo-controller-js', plugins_url('/slider-pro/js/slider/dev/froogaloop.js'), $deps, SLIDER_PRO_VERSION);	
	...
	if (in_array('videojs_controller', $sliderpro_scripts_to_load) && !sliderpro_is_old_ie()) {
		if (get_option('slider_pro_use_compressed_scripts'))
			wp_register_script('slider-pro-videojs-controller-js', plugins_url('/slider-pro/js/slider/video.min.js'), $deps, SLIDER_PRO_VERSION);
		else
			wp_register_script('slider-pro-videojs-controller-js', plugins_url('/slider-pro/js/slider/dev/video.js'), $deps, SLIDER_PRO_VERSION);		
	...
	if (in_array('jwplayer_controller', $sliderpro_scripts_to_load)) {
		wp_register_script('slider-pro-jwplayer-controller-js', $sliderpro_scripts_to_load['jwplayer_path'] . '/jwplayer.js', $deps, SLIDER_PRO_VERSION);
	...
	if (count(array_intersect(array('youtube_controller', 'vimeo_controller', 'html5_controller', 'videojs_controller', 'jwplayer_controller'), $sliderpro_scripts_to_load))) {
		if (get_option('slider_pro_use_compressed_scripts'))
			wp_register_script('slider-pro-video-controller-js', plugins_url('/slider-pro/js/slider/jquery.videoController.min.js'), $deps, SLIDER_PRO_VERSION);
		else
			wp_register_script('slider-pro-video-controller-js', plugins_url('/slider-pro/js/slider/dev/jquery.videoController.js'), $deps, SLIDER_PRO_VERSION);		
	...
	if (in_array('touch_swipe', $sliderpro_scripts_to_load)) {
		if (get_option('slider_pro_use_compressed_scripts'))
			wp_register_script('slider-pro-touch-swipe-js', plugins_url('/slider-pro/js/slider/jquery.touchSwipe.min.js'), $deps, SLIDER_PRO_VERSION);
		else
			wp_register_script('slider-pro-touch-swipe-js', plugins_url('/slider-pro/js/slider/dev/jquery.touchSwipe.js'), $deps, SLIDER_PRO_VERSION);
	...
	if (in_array('slider', $sliderpro_scripts_to_load)) {
		if (get_option('slider_pro_enqueue_jquery_easing')) {
			if (get_option('slider_pro_use_compressed_scripts'))
				wp_register_script('slider-pro-slider-easing-js', plugins_url('/slider-pro/js/slider/jquery.easing.1.3.min.js'), $deps, SLIDER_PRO_VERSION);
			else
				wp_register_script('slider-pro-slider-easing-js', plugins_url('/slider-pro/js/slider/dev/jquery.easing.1.3.js'), $deps, SLIDER_PRO_VERSION);
				
			wp_print_scripts('slider-pro-slider-easing-js');
		}
		
		if (get_option('slider_pro_use_compressed_scripts'))
			wp_register_script('slider-pro-slider-js', plugins_url('/slider-pro/js/slider/jquery.advancedSlider.min.js'), $deps, SLIDER_PRO_VERSION);
		else
			wp_register_script('slider-pro-slider-js', plugins_url('/slider-pro/js/slider/dev/jquery.advancedSlider.js'), $deps, SLIDER_PRO_VERSION);
	...
	foreach ($sliderpro_custom_scripts_to_load as $script_id) {
		wp_register_script('slider-pro-' . $script_id . '-custom-js', plugins_url('/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $script_id . '/slider-pro-' . $script_id . '-custom-js.js'));
...
function sliderpro_init() {		
	global $sliderpro_slider_skins, $sliderpro_scrollbar_skins, $sliderpro_all_skins, $sliderpro_unique_id, $sliderpro_used_ids, $sliderpro_sliders_js,
		   ...
		   $sliderpro_videojs_loaded, $sliderpro_videojs_found, $sliderpro_flickr, $sliderpro_video_unique_id, $sliderpro_lightbox_slider, $sliderpro_assets_dir;
	...
	$sliderpro_assets_dir = get_option('slider_pro_is_assets_dir') == 'yes' ? 'slider-pro-assets' : 'slider-pro';
...
function sliderpro_admin_init() {
	if (sliderpro_is_slider_pro_admin()) {
		...
		global $wpdb, $sliderpro_current_action, $sliderpro_current_slider_id, $sliderpro_assets_dir;
		...
		if ($sliderpro_current_action == 'create' || $sliderpro_current_action == 'update') {
			...
			if ($sliderpro_current_action == 'create') {
				...
				delete_option( 'sliderpro_sliders_list' );
				...
				$wpdb->query($wpdb->prepare("DELETE FROM " . $wpdb->prefix . "sliderpro_slides WHERE slider_id = %d", $sliderpro_current_slider_id));
		...
		if ($sliderpro_current_action == 'delete' && wp_verify_nonce($_GET['_wpnonce'], 'delete-slider')) {
			$wpdb->query($wpdb->prepare("DELETE FROM " . $wpdb->prefix . "sliderpro_slides WHERE slider_id = %d", $sliderpro_current_slider_id));
			$wpdb->query($wpdb->prepare("DELETE FROM " . $wpdb->prefix . "sliderpro_sliders WHERE id = %d", $sliderpro_current_slider_id));
		...
		if ($sliderpro_current_action == 'duplicate_slider' && wp_verify_nonce($_GET['_wpnonce'], 'duplicate-slider')) {
			...
			if ($custom_css) {
				$original_file_path = WP_PLUGIN_DIR . '/'. $sliderpro_assets_dir. '/custom/slider-pro-' . $sliderpro_current_slider_id . '/slider-pro-' . $sliderpro_current_slider_id . '-custom-css.css';
				$copy_file_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $new_id . '/slider-pro-' . $new_id . '-custom-css.css';
			...
			if ($custom_js) {
				$original_file_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $sliderpro_current_slider_id . '/slider-pro-' . $sliderpro_current_slider_id . '-custom-js.js';
				$copy_file_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $new_id . '/slider-pro-' . $new_id . '-custom-js.js';
		...
		if (isset($_POST['import-slider-submit'])) {
			...
			foreach ($_FILES['import-slider-file']['tmp_name'] as $xml_file_name) {
				...
				if ($custom_css) {
					...
					$css_file_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $sliderpro_current_slider_id . '/slider-pro-' . $sliderpro_current_slider_id . '-custom-css.css';
				...
				if ($custom_js) {
					...
					$js_file_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $sliderpro_current_slider_id . '/slider-pro-' . $sliderpro_current_slider_id . '-custom-js.js';
...
function sliderpro_add_tinymce_plugin($plugin_array) {
	$file = floatval(get_bloginfo('version')) >= 3.9 ? plugins_url('/slider-pro/js/editor-plugin.js') : plugins_url('/slider-pro/js/editor-plugin-old.js');
	$plugin_array['sliderpro'] = $file;
	return $plugin_array;
}
...
function sliderpro_sliders_ui() {
	global $sliderpro_current_action, $sliderpro_current_slider_id, $sliderpro_assets_dir;
...
function sliderpro_skin_editor_ui() {
	...
	if (isset($_POST['replicate_skin'])) {
		...
		$destination_dir = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/skins/' . $type;		
		$new_skin_url = plugins_url('/' . $sliderpro_assets_dir . '/skins/' . $type . '/' . $skin_class . '/' . $skin_class . '.css');
...
function sliderpro_plugin_options_ui() {
	...
	if ( isset( $_POST['purchase_code_update'] ) ) {
		...
		if ( isset( $_POST['purchase_code'] ) ) {
			...
			update_option( 'slider_pro_purchase_code', $purchase_code );

			if ( $_POST['purchase_code'] === '' ) {
				$purchase_code_status = '0';
			} else {
				$verification_result = sliderpro_verify_purchase_code( $purchase_code );
			...
			update_option( 'slider_pro_purchase_code_status', $purchase_code_status );
		}
	}

	if ( isset( $_POST['prepare_plugin'] ) ) {
		check_admin_referer( 'prepare-plugin-update', 'prepare-plugin-nonce' );

		$assets_dir = WP_PLUGIN_DIR . '/slider-pro-assets';
		$is_xml_dir = true;
		$is_skins_dir = true;
		$is_custom_dir = true;
		...
		if (!is_dir(WP_PLUGIN_DIR . '/slider-pro-assets/xml'))
			$is_xml_dir = sliderpro_copy_dir(WP_PLUGIN_DIR . '/slider-pro/xml', WP_PLUGIN_DIR . '/slider-pro-assets/xml');
		
		if (!is_dir(WP_PLUGIN_DIR . '/slider-pro-assets/skins'))
			$is_skins_dir = sliderpro_copy_dir(WP_PLUGIN_DIR . '/slider-pro/skins', WP_PLUGIN_DIR . '/slider-pro-assets/skins');
		
		if (!is_dir(WP_PLUGIN_DIR . '/slider-pro-assets/custom'))
			$is_custom_dir = sliderpro_copy_dir(WP_PLUGIN_DIR . '/slider-pro/custom', WP_PLUGIN_DIR . '/slider-pro-assets/custom');

		if ($is_xml_dir == true && $is_skins_dir == true && $is_custom_dir == true) {
			update_option( 'slider_pro_is_assets_dir', 'yes' );
			$sliderpro_assets_dir = 'slider-pro-assets';
			sliderpro_refresh_skins(array('slider', 'scrollbar'));
		} else {
			update_option( 'slider_pro_is_assets_dir', 'no' );
		}
	}
...
function sliderpro_get_slider($id) {
	...
	$slider_raw = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE id = %d", $id), ARRAY_A);
...
function sliderpro_get_slides($id) {
	...
	$slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE slider_id = %d ORDER BY position", $id), ARRAY_A);
...
function sliderpro_duplicate_slide() {
	...
	if ($id != -1) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'sliderpro_slides';
		$slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_name WHERE id = %d", $id), ARRAY_A);
...
function sliderpro_create_slide_setting_field($setting_name, $setting_value, $counter) {
	...
	$setting_field_name = '<label title="' . $setting_name . '"><span>' . __($setting['name'], 'slider_pro') . '</span></label>';
...
function sliderpro_slider_preview() {
	...
	echo slider_pro($id);
	sliderpro_load_slider_scripts( true );
...
function sliderpro_tinymce_plugin() {
	if ( ( $sliders_list = get_option( 'sliderpro_sliders_list' ) ) === false ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'sliderpro_sliders';
		$sliders = $wpdb->get_results("SELECT id, name FROM $table_name");

		$sliders_list = json_encode(stripslashes_deep($sliders));

		update_option( 'sliderpro_sliders_list', $sliders_list );
	}
...
function sliderpro_edit_custom_js_css() {
	...
	$file_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $id . '/slider-pro-' . $id . '-custom-' . $file_type . '.' . $file_type;
...
function sliderpro_save_custom_js_css() {
	...
	$file_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $id . '/slider-pro-' . $id . '-custom-' . $type . '.' . $type;
...
function sliderpro_slider_generate_xml($id) {
	...
	if (!get_option('slider_pro_generate_xml_file') || !is_writable(WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/xml'))
		return;
	...
	$css_file_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $id . '/slider-pro-' . $id . '-custom-css.css';
	$js_file_path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/custom/slider-pro-' . $id . '/slider-pro-' . $id . '-custom-js.js';
	...
	$path = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/xml/' . $name;
	$xml->save($path);
}
...
function slider_pro($id, $atts = null, $content = null) {
	...
	$posts_terms_pattern = sliderpro_regex(array('sp_image', 'sp_image_alt', 'sp_image_title', 'sp_image_caption', 'sp_image_description', 'sp_comments_number', 'sp_category', 
												 ...
												 'sp_image_width', 'sp_image_height'));
	...
	foreach ($slides as $slide) {
		...
		if (sliderpro_get_slide_setting($slide['settings'], 'slide_type', 'dynamic') == 'posts') {
						...
						foreach ($slide_dynamic_terms as $term => $term_data) {
							...
							if (strpos($field_content, $term) !== false) {								
								...
								if ($term_data['term_name'] == 'sp_image') {
									...
									$replace = !empty($categories) ? $categories[0]->name : '';
								...
								} else if ($term_data['term_name'] == 'sp_image_width') {		
									$image_size = isset($term_data['size']) ? $term_data['size'] : 'full';
									$image_src = wp_get_attachment_image_src($image_id, $image_size);

									$replace = $image_src[1];
									
								}  else if ($term_data['term_name'] == 'sp_image_height') {		
									$image_size = isset($term_data['size']) ? $term_data['size'] : 'full';
									$image_src = wp_get_attachment_image_src($image_id, $image_size);
			...
			if ($dynamic_gallery_type == 'gallery-images') {
				...
				$current_post = get_post($gallery_post_id);
				...
				$gallery_images_result = get_children($children_args, ARRAY_A);
				$gallery_images_values = array_values($gallery_images_result);
				$gallery_images = array_splice($gallery_images_values, $dynamic_gallery_offset);
			}
	...
	foreach ($slides as $index => $slide) {
		...
		if ($slide_js_properties != "") {
			...
			$slides_js_properties .= SP_IND_4 . $slide_counter . ': {' . $slide_js_properties . '}';	
		}
	...
	$slider_classes = sliderpro_get_setting($slider_settings, 'custom_class') != '' ? 'advanced-slider slider-pro ' . sliderpro_get_setting($slider_settings, 'custom_class') : 'advanced-slider slider-pro';
...
function sliderpro_refresh_skins($skin_types) {
	global $wpdb, $sliderpro_assets_dir;

	$wpdb->query("TRUNCATE TABLE " . $wpdb->prefix . "sliderpro_skins");
	
	foreach ($skin_types as $type) {
		if (!isset($sliderpro_assets_dir))
			$sliderpro_assets_dir = 'slider-pro';
		
		$skins_root_dir = WP_PLUGIN_DIR . '/' . $sliderpro_assets_dir . '/skins/' . $type;
		...
		foreach ($regex as $item) {
			...
			$file_directory_array = explode('/' . $sliderpro_assets_dir . '/skins/' . $type . DIRECTORY_SEPARATOR, $path);
			...
			$skin_url = plugins_url('/' . $sliderpro_assets_dir . '/skins/' . $type . '/' . $file_directory . '/' . $file_name);
...
function sliderpro_get_skins($type) {	
	...
	$skins_raw = $wpdb->get_results($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . "sliderpro_skins WHERE type = '%s'", $type), ARRAY_A);
...
function sliderpro_get_skins_for_slider($id) {
	...
	global $wpdb;
	$prefix = $wpdb->prefix;
	$slider_configs = $wpdb->get_results($wpdb->prepare("SELECT settings FROM " . $prefix . "sliderpro_sliders WHERE id = %d", $id), ARRAY_A);
...
function sliderpro_copy_dir($origin, $destination) {
	if (!is_dir($origin))
		return false;

	$origin_handle = opendir($origin);
	
	$success = @mkdir($destination, 0755, true);
	...
	while ($resource = readdir($origin_handle)) {
		...
		if (is_dir($origin . '/' . $resource)) {
			sliderpro_copy_dir($origin . '/' . $resource, $destination . '/' . $resource);
		} else {
			copy($origin. '/' . $resource, $destination . '/' . $resource);
		}
	}
