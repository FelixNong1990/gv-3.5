...
function taqyeem_get_user_rate(){
	...
	if ( $user_ID ) {
		...
		if( !empty($user_rated) && is_array($user_rated) && array_key_exists($post->ID , $user_rated) ){
			...
			return $output = 
			'<div class="user-rate-wrap" itemprop="reviewVideo" itemscope itemtype="http://schema.org/Video">
				<meta itemprop="worstRating" content = "1" />
				<meta itemprop="bestRating" content = "100" />
				<span class="rating points" style="display:none">
					<span class="rating points" itemprop="ratingValue">'. $user_rate .'</span>
				</span>
