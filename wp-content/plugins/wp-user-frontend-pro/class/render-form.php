...
class WPUF_Render_Form {
    ...
    function label( $attr, $post_id = 0 ) {
        ...
        <label class="col-lg-2 control-label" for="wpuf-<?php echo isset( $attr['name'] ) ? $attr['name'] : 'cls'; ?>"><?php echo $attr['label'] //. $this->required_mark( $attr ); ?></label>
    ...
    function text( $attr, $post_id, $type = 'post' ) {
        ...
        <div class="col-lg-8">
            ...
            <?php 
				if ( $taxonomy ) { 
					global $wpdb;
					$results = $wpdb->get_results("SELECT * FROM $wpdb->term_taxonomy WHERE taxonomy = 'post_tag'",ARRAY_A);
					// echo "<pre>";
					// print_r($results);
					// echo "</pre>";
					?>
					...
					<script type="text/javascript">
						jQuery(function($) {
							...
							$('#tags').on('tokenfield:createtoken', function (e) {
								var newVal = e.attrs.value.replace(/\s+/g, '').toLowerCase(),
									orgVal = this.value,
									trimOrgVal = orgVal.replace(/\s+/g, ''),
									orgValArr = trimOrgVal.toLowerCase(trimOrgVal).split(',');
								
								if(newVal.split(' ').length > 3 || newVal.length > 20) {
								    alertify.error("The keyword is too long");
									e.preventDefault();
								} else if($.inArray(newVal, orgValArr) > -1) {
									alertify.error("You've already added this tag");
									e.preventDefault();
								}
							});
						});
    ...
    function textarea( $attr, $post_id, $type ) {
        ...
        <div class="col-lg-8">
            ...
            if ( $attr['rich'] == 'yes' ) {
                ...
                printf( '<span class="wpuf-rich-validation" data-required="%s" data-type="rich" data-id="%s"></span>', $attr['required'], $attr['name'] );
                wp_editor( $value, $attr['name'], array('editor_height' => $attr['rows'], 'quicktags' => false, 'media_buttons' => false, 'teeny' => true, 'editor_class' => $req_class, tinymce => array('theme' =>  'modern', 'plugins' => 'paste' ,'paste_text_sticky' => true, 'paste_text_sticky_default'=> true)) );
            } else {
                ?>
        ...
        <div class="col-lg-8">
            ...
            switch ($attr['type']) {
                ...
                case 'multiselect':
                    ...
                    echo str_replace( '<select', '<select style="width:400px;" multiple="multiple" ' . $required, $select );
                    break;
