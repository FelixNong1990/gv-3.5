...
class WPUF_Walker_Category_Multi extends Walker {
    ...
    function start_el( &$output, $category, $depth, $args, $id = 0 ) {
        $pad = str_repeat( '&ensp;', $depth);

        $cat_name = apply_filters( 'list_cats', $category->name, $category );
        //$parentcat_name = get_category_parents( $category->term_id);
        $parentCatList = get_category_parents($category,false,',');
        ...
        if ($depth == 0) {
            ...
            $output .= "<option class='" . "level-" . $depth . "' 
                        value='" . $category->term_id . "' data-parent='" . $topParentName . "'";
